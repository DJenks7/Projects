...
// Class listnode, contains an element called data and a listnode pointer called next.
class listnode {
public:
	element data;
	listnode * next;
...
// Class Llist, contains constructor and destructor, two listnode pointers called head and tail, and methods
//	Action, Message, Duplicate, InsertTail, Clean, Help, Print, Read, CaesarEncrypt, CaesarDecrypt, VigenereEncrypt, and VigenereDecrypt.
class Llist {
public:
	Llist();
	~Llist();
	listnode * head;
	listnode * tail;
	void Action();						// Holds loop to keep awaiting input from user and menu displaying.
	void Message();						// Holds users Current Message and asks for user input.
...
private:
	...
	void Read();				// Reads a new message to become the Current Message.
	void CaesarEncrypt();		// Code to encrypt the Current Message via the Caesar Encryption.
	void CaesarDecrypt();		// Code to decrypt the Current Message via the Caesar Decryption.
	void VigenereEncrypt();		// Code to encrypt the Current Message via the Vigenere Encryption.
	void VigenereDecrypt();		// Code to decrypt the Current Message via the Vigenere Decryption.
};
...
Llist::~Llist() {
	// PRE: N.O. is valid.
	// POST: N.O. is still valid and is empty. Memory has been returned to heap.
	Clean();
}

void Llist::Action() {
	// PRE: N.O. is valid.
	// POST: The program has been run, the current message has been displayed at least once.
	//		The user has entered q to exit the program.
	element action;

	Message();              // Prints the Current Message before awaiting user input.
...
void Llist::CaesarEncrypt() {
	//PRE: N.O. is valid.
	//POST: N.O.'s listnodes have been encrypted by the Key entered by the user. PlainText, Key, and CipherText have been displayed.
	...
	cout << "Cipher Text: ";
	Print();
	cout << "\n\nCaesar Encryption Complete.\n";
}

void Llist::CaesarDecrypt() {
	//PRE: N.O. is valid.
	//POST: N.O.'s listnodes have been decrypted by the Key entered by the user. CipherText, Key, and PlainText have been displayed.
	//      Llist Cipher and OldText has been used and properly disposed of.
	int Key = 0;
	listnode * temp;
	Llist OldText;
...
void Llist::Duplicate(Llist & Source) {
	...
	//POST: N.O. Llist's original listnodes have been cleaned, and N.O. Llist is now a listnode by listnode copy of the Source Llist.
...
void Llist::Help() {
	...
	cout << "\nValid commands are:\n";
	cout << "m      enter a new current message from the keyboard.\n";
...
void Llist::InsertTail(element thing) {
	...
	//POST: N.O. is unchanged except it has 1 new listnode at tail end, containing element thing.
	listnode * temp;
...
void Llist::Message() {
	//PRE: None.
	//POST: Current message has been displayed. Waiting on user input.
	cout << "\nCurrent Message: \"";
	Print();
	cout << "\".\n\n";
...
void Llist::Read() {
	//PRE: N.O. Llist is valid.
	//POST: N.O. is still valid, using data entered by the user and elements will be in order entered.
	element User_Val;
	listnode * temp;

	Clean();
	cout << "\nUse Enter/Return button to submit.\n";
	cout << "Enter text: ";
	cin.ignore(80, '\n');
