...
void Alist::BubbleSort() {
	...
	cout << "Sort statistics: " << Acount << " element comparisons, " << Amove << " element movements\n\n";
}
...
void Alist::LinearSearch(element Target, bool & found, int & Pos) {
	...
	cout << "Search statistics: " << Acount << " element comparisons\n";
...
void Alist::BinarySearch(element Target, bool & found, int & Pos) {
	...
	if (sorted == true) {
		...
		// If the Target element was found.
		if (found == true) {
			cout << "\nTarget (" << Target << ") was FOUND at list position " << Pos + 1 << "\n\n";
		}
...
void Alist::Action() {
	...
	element search;      // Elements for user input for Searching Methods.
	bool Found;                     // Boolean for if element search is found.
	int action = 1, pos, Size;		// Integers, action is for menu input, pos is for Search method to display
